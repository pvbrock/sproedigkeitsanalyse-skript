...
for sheet_name in probe_sheets:
    ...
    # Berechnung des MSE für die Spline-Glättung
    mse_spline = np.mean((y_filtered - y_smooth_spline) ** 2)
    print(f"MSE der Spline-Glättung: {mse_spline:.4f}")
    ...
    # Berechnung des MSE der linearen Regression im betrachteten Bereich
    y_pred_regression = linear_model.predict(x_regression)
    mse_linear = np.mean((y_regression - y_pred_regression) ** 2)
    print(f"MSE der linearen Regression: {mse_linear:.4f}")
    ...
    if len(close_indices_bw) > 0:
        ...
        print(f"Rückwärts: Ab x={close_x:.4f} mm ist Abweichung < {close_threshold * 100:.1f}% (Spline={close_spline_y:.4f} N)")
    else:
        close_x = None
    ...
    gesamt_ergebnisse.append({
        ...
        "Lineare Regression MSE": mse_linear,
    ...
    # Annotiere Sprödigkeitsindex, MSE (Spline & Linear Regression), und E-Modul (in GPa) im Plot
    plt.text(0.5, 0.95, f"Sprödigkeitsindex: {sprödigkeitsindex:.2f}%",
             fontsize=12, color='darkred', ha='center', va='top', transform=plt.gca().transAxes)
    plt.text(0.5, 0.90, f"MSE (Spline): {mse_spline:.4f}",
             fontsize=12, color='blue', ha='center', va='top', transform=plt.gca().transAxes)
    plt.text(0.5, 0.85, f"MSE (Linear Regression): {mse_linear:.4f}",
             fontsize=12, color='green', ha='center', va='top', transform=plt.gca().transAxes)
    if computed_E_modul_GPa is not None:
        plt.text(0.5, 0.80, f"E-Modul: {computed_E_modul_GPa:.2f} GPa",
...
output_file = f"Analyse_Ergebnisse_Lineare_Regression_BU_CA_smooth({spline_smooth_factor})_schwelle({close_threshold})re({regression_start_point_mm}-{regression_end_point_mm}).xlsx"
