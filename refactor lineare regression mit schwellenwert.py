...
for sheet_name in probe_sheets:
    ...
    # -------------------------------------------------------------
    regression_mask = (x_filtered >= 0.5) & (x_filtered <= 3.5)
    ...
    regression_end_point = (3.5, slope * 3.5 + intercept)
    ...
    plt.plot(regression_start_point[0], regression_start_point[1], 'go', label='Regression Startpunkt')
    plt.plot(regression_end_point[0], regression_end_point[1], 'mo', label='Regression Endpunkt')


    # (c) Punkt Annäherung (rückwärts)
